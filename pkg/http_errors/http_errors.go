...
const (
	...
	ErrInvalidGiftcharge = "Invalid giftcharge"
	ErrInvalidPassword   = "Invalid password"
	ErrInvalidField      = "Invalid field"
	DuplicateRequest     = "you received this gift before"
)
...
var (
	...
	ExistsEgiftchargeError = errors.New("User with given giftcharge already exists")
...
func ParseErrors(err error) RestErr {
	...
	case strings.Contains(strings.ToLower(err.Error()), "phone validation"):
		return parseValidatorError(err)
	case strings.Contains(strings.ToLower(err.Error()), "you received this gift before"):
		return NewRestError(http.StatusBadRequest, DuplicateRequest, err)
...
func parseValidatorError(err error) RestErr {
	...
	if strings.Contains(err.Error(), "giftCharge") {
		return NewRestError(http.StatusBadRequest, ErrInvalidGiftcharge, err)
	}
